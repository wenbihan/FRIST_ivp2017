function [TE, IDX] = rotateFRIST_merge_SPIEl0(TE, W, permutation, threshold, isFlipping, selectIndex)
...
for k = 1 : K
    if ismember(k, selectIndex)
        a1 = TE(L(:,k), :);
        a1 = W * a1;
%         a0 = sparseSail0p(a1, threshold);
        a0 = sparse_SPIEl0(a1, threshold);
        error(k, :) = sum((a1 - a0).^2) + (threshold^2) * (sum(abs(a0)~=0));
    else
        error(k, :) = flintmax;
    end
    if isFlipping
