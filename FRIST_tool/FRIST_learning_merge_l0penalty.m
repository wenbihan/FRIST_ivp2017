function [YH, D, outputParam]= FRIST_learning_merge_l0penalty(TE, D, param)
...
%                   - frac:      fraction of code to be sparse.
%
%
% Outputs -
...
if showStats
    sp = zeros(iter + 1, 1);
    sp(1, 1) = SparsificationError_sail0p(TE, D, frac);
    if ~isUnitary
        ct = zeros(iter + 1, 1);
...
for j =1:iter
%%%%%%%%%%% clustering %%%%%%%%%%
[YH, IDX] = rotateFRIST_merge_sail0p(TE, D, permutation, frac, isFlipping, selectIndex);
...
%     end
    if isfield(param, 'isUnitary') && isUnitary
        D = forwardORTHOTRANSb_sail0p(D,YH,numiter,frac);
    else
        D = forwardDL3LearningonlyconjualtcostEXACT2_sail0p(D, YH, numiter, l2,l2, frac);  %param.L is not used in subfunction
    end
    if showStats
        numIter = j + 1;
        sp(numIter, 1) = SparsificationError_sail0p(YH, D, frac);
        if ~isUnitary
